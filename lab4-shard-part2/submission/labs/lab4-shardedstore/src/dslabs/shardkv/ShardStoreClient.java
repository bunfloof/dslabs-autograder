...
public class ShardStoreClient extends ShardStoreNode implements Client {
  ...
  private int shardMasterCursor = 0;
  /* -----------------------------------------------------------------------------------------------
  ...
  private void sendQuery() {
    querySeqNum++;
    Address[] masters = shardMasters();
    if (masters == null || masters.length == 0) return;
    int idx = Math.floorMod(shardMasterCursor, masters.length);
    shardMasterCursor++;
    send(new PaxosRequest(new AMOCommand(new Query(-1), address(), querySeqNum)), masters[idx]);
  }
