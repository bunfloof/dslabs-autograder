...
public class PaxosServer extends Node {
  ...
  private HashSet<AMOCommand> pendingCommands;
  ...
  private void handlePaxosRequest(PaxosRequest m, Address sender) {
    ...
    if (parentAddress == null && app.alreadyExecuted(cmd)) {
  ...
  private void executeLog() {
    while (chosenSlots.contains(slotOut)) {
        ...
        if (cmd != null) pendingCommands.remove(cmd);
        if (parentAddress != null) send(new PaxosDecision(slotOut, cmd), parentAddress);
        else if (cmd != null) {
            AMOResult result = app.execute(cmd);
  ...
  private void proposeIfNeeded(AMOCommand cmd) {
    if (parentAddress == null && app.alreadyExecuted(cmd)) {
    ...
    proposeNewCommand(cmd);
  }

  private void proposePendingCommands() {
    if (!active || pendingCommands.isEmpty()) return;
    HashSet<AMOCommand> copy = new HashSet<>(pendingCommands);
    for (AMOCommand cmd : copy) proposeIfNeeded(cmd);
  }
